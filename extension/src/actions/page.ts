    ...
    const results = await chrome.scripting.executeScript({
      ...
      func: () => {
        ...
        const removeSelectors = [
          ...
          "textarea", "nav", "footer", "header[role='banner']",
          "[role='navigation']", "[role='complementary']", "[role='contentinfo']",
          ".ad", ".ads", ".advertisement", ".sidebar", ".nav", ".menu",
          ".footer", ".header", ".comment", ".comments", ".social",
          ".share", ".popup", ".modal", ".cookie-banner", ".cookie-notice",
        ];
        ...
        function htmlToMarkdown(el: Element, indent = 0): string {
          ...
          for (const child of Array.from(el.childNodes)) {
            if (child.nodeType === Node.TEXT_NODE) {
              ...
              switch (childTag) {
                ...
                case "blockquote":
                  childEl.textContent?.trim().split("\n").forEach((line) => {
                    lines.push(`> ${line}`);
                  });
                  break;
                default:
          ...
          const blockTags = ["h1","h2","h3","h4","h5","h6","p","ul","ol","pre","table","blockquote","hr","div","section","article"];
...
export async function handlePageExecute(params: {
  ...
  const res = results[0].result as { success: boolean; value: unknown; error?: string };
  if (!res.success) {
...
export async function handlePageSelect(params: {
  ...
  return { tabId: params.tabId, selector: params.selector, value: params.value };
}
...
export async function handlePageScroll(params: {
  ...
  await chrome.scripting.executeScript({
    target: { tabId: params.tabId },
    func: (sel: string | undefined, x: number | undefined, y: number | undefined) => {
      if (sel) {
...
export async function handlePageQuery(params: {
  ...
  const results = await chrome.scripting.executeScript({
    ...
    args: [params.selector, params.attributes ?? null],
  });
        ...
        for (const node of Array.from(el.childNodes)) {
          if (node.nodeType === Node.TEXT_NODE) {
            ...
            switch (tag) {
              ...
              case "strong": case "b": parts.push(`**${child.textContent?.trim()}**`); break;
              case "em": case "i": parts.push(`*${child.textContent?.trim()}*`); break;
              case "code": parts.push(`\`${child.textContent?.trim()}\``); break;
              case "pre": parts.push(`\n\n\`\`\`\n${child.textContent}\n\`\`\`\n\n`); break;
              case "ul": case "ol": {
              ...
              default: parts.push(htmlToMarkdown(child, indent)); break;
            }
          }
        }
        return parts.join("");
